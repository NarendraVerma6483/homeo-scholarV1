...
// Seed remedies data for development/demo when backend is empty
const SEED_REMEDIES: Remedy[] = [
  {
    id: "ars-alb",
    ...
    constitution:
      "Thin, anxious, restless individual who is chilly and fearful",
    clinicalIndications: [
      "Food poisoning",
      "Asthma",
      "Eczema",
  ...
  },
  {
    ...
    constitution:
      "Robust, full-blooded individual with a vigorous, reactive constitution",
    clinicalIndications: [
      "High fever",
      "Acute inflammations",
      "Scarlet fever",
  ...
  },
  {
    ...
    constitution:
      "Fair, fat, flabby individual; children with big belly and big head",
    clinicalIndications: [
      "Hypothyroidism",
      "Obesity",
      "Eczema",
  ...
  },
  {
    ...
    constitution:
      "Lean, intellectual individual with a yellowish, wrinkled appearance",
    clinicalIndications: [
      "Liver disorders",
      "IBS",
      "Urinary complaints",
  ...
  },
  {
    ...
    constitution:
      "Thin, nervous, ambitious individual prone to overwork and stimulants",
    clinicalIndications: [
      "Hangover",
      "Indigestion",
      "Constipation",
  ...
  },
  {
    ...
    constitution:
      "Lean, stooped, dirty-looking person; or red-faced, plethoric individual",
    clinicalIndications: [
      "Skin conditions",
      "Eczema",
      "Psoriasis",
...
export function useListRemedies() {
  ...
  return useQuery<Remedy[]>({
    ...
    queryFn: async () => {
      ...
      try {
        const result = await (
          backend as unknown as { listRemedies: () => Promise<Remedy[]> }
        ).listRemedies();
...
export function useGetRemedy(id: string) {
  const { backend, isLoading: backendLoading } = useBackend();
  return useQuery<Remedy | undefined>({
    queryKey: ["remedy", id],
    queryFn: async () => {
...
export function useSearchRemediesByName(query: string) {
  const { backend, isLoading: backendLoading } = useBackend();
  return useQuery<Remedy[]>({
    ...
    queryFn: async () => {
      ...
      try {
        return await (
          backend as unknown as {
            searchRemediesByName: (q: string) => Promise<Remedy[]>;
          }
        ).searchRemediesByName(query);
      } catch {
        return SEED_REMEDIES.filter((r) =>
          r.name.toLowerCase().includes(lower),
        );
      }
    },
...
export function useSearchRemediesBySymptom(symptom: string) {
  const { backend, isLoading: backendLoading } = useBackend();
  return useQuery<Remedy[]>({
    ...
    queryFn: async () => {
      ...
      try {
        return await (
          backend as unknown as {
            searchRemediesBySymptom: (s: string) => Promise<Remedy[]>;
          }
        ).searchRemediesBySymptom(symptom);
      } catch {
        return SEED_REMEDIES.filter((r) =>
          r.keySymptomsPhysical.some((s) => s.toLowerCase().includes(lower)),
        );
      }
    },
...
export function useSeedRemedies() {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      return await (
        backend as unknown as { seedRemedies: () => Promise<void> }
      ).seedRemedies();
    },
    onSuccess: () => {
